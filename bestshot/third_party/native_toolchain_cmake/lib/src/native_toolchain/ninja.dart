...
class _NinjaResolver implements ToolResolver {
  ...
  Future<List<ToolInstance>> resolve({
    ...
    var androidNinjaInstances = await androidResolver.resolve(
      logger: logger,
  ...
  /// When glob fails on Windows, discover `Sdk/cmake/<ver>/bin/ninja(.exe)`.
  Future<List<ToolInstance>> _ninjaInstancesFromAndroidSdkCmake(
    String sdkRoot,
    String executableName,
    Logger? logger,
  ) async {
    final cmakeRoot = Directory(p.join(sdkRoot, 'cmake'));
    if (!await cmakeRoot.exists()) return [];
    final out = <ToolInstance>[];
    await for (final entity in cmakeRoot.list(followLinks: false)) {
      if (entity is! Directory) continue;
      final ninjaFile = File(p.join(entity.path, 'bin', executableName));
      if (!await ninjaFile.exists()) continue;
      out.add(
        await CliVersionResolver.lookupVersion(
          ToolInstance(tool: ninja, uri: ninjaFile.uri),
          logger: logger,
        ),
      );
    }
